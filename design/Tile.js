...
function Tile(props) {
  ...
  const sendmail = ()=>{
    let name = props.orderid;
    let email = props.uid;
    let message = `Your Order ${props.orderid} of cost ₹${cost} has been taken up by ${user?.uid}.
          click on orderbyus.herokuapp.com/ongoingOrders for more details
  ...
  function checkBox(index) {
    ...
    if (temp[index] == 0) {
      temp[index] = 1;
      var intvalue = Math.trunc(cost - (props.orderlist[index].cost + (props.orderlist[index].cost * 0.1)));
      // let toint = (int)();
  ...
  return (
    <div className={styles.card2} id={"card" + props.orderid} >
      ...
      <div className={styles.dropdown}>
              ...
              <p>
                {availability == 0 ? (
                  <label className={styles.container}>
                    <input type="checkbox" className={styles.container}  onClick={() => checkBox(index, 0)} />
                  </label>
                ) : (
                  <label className={styles.container}>
                      <input type="checkbox" className={styles.container} checked="checked" onClick={() => checkBox(index, 1)} />
                  </label>
                )}
              </p>
            ))}
        ...
        <div className={styles.mobileNumber}>
          ...
          <span className={styles.buttonn} onClick={disableControl}>
            {Number ? <span>Change</span> : <span>{Number == number ? "Add" : "Get OTP"}</span>}
          </span>
      ...
      <div className={styles.toggle} onClick={fun}>
